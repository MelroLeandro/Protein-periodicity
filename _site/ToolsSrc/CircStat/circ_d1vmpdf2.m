function [p N_ang] = circ_d1vmpdf2(N_ang, phi_i,psi_i, kappa1,kappa2, lambda)
...
for i=1:N_ang
    ...
    for j=1:N_ang
        sum=0;
        if dim==2
            if lambda(i,j)<0.5e-1
                lambda(i,j)=0.5e-1;
            end
            for k=1:N_data
                C = 1/(4*pi^2*N_data*besseli(0,kappa1/lambda(i,j))*besseli(0,kappa2/lambda(i,j)));
                sum=sum - C*exp(kappa1/lambda(i,j)*cos(phi-phi_i(k))+kappa2/lambda(i,j)*cos(psi-psi_i(k)))*kappa1/lambda(i,j)*sin(phi-phi_i(k));
            end
            p(i,j) = sum;
        else
            for k=1:N_data
                sum=sum - exp(kappa1*cos(phi-phi_i(k))+kappa2*cos(psi-psi_i(k)))*kappa1*sin(phi-phi_i(k));
            end
