function [p N_ang] = circ_d1vmpdf2Test(N_ang, phi_i,psi_i, kappa, lambda)
    ...
    for j=1:N_ang
        ...
        if dim==2
            for k=1:N_data
                ...
                sum=sum + C*exp(kappa/lambda(k)*cos(phi-phi_i(k))+cos(psi-psi_i(k)))*kappa/lambda(k)*sin(phi-phi_i(k));
            end
            p(i,j) = sum;
        else
            for k=1:N_data
                  sum=sum + exp(kappa*cos(phi-phi_i(k))+cos(psi-psi_i(k)))*kappa*sin(phi-phi_i(k));
            end
