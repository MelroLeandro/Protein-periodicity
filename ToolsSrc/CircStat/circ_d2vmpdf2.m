function [p N_ang] = circ_d2vmpdf2(N_ang, phi_i,psi_i, kappa1, kappa2, lambda)
    ...
    for j=1:N_ang
        ...
        if dim==2
            ...
            for k=1:N_data
                ...
                sum=sum - C*exp(kappa1/lambda(i,j)*cos(phi-phi_i(k))+kappa2/lambda(i,j)*cos(psi-psi_i(k)))*kappa2/lambda(i,j)*sin(psi-psi_i(k));
            end
            p(i,j) = sum;
        else
            for k=1:N_data
                sum=sum - exp(kappa1*cos(phi-phi_i(k))+kappa2*cos(psi-psi_i(k)))*sin(psi-psi_i(k));
            end
